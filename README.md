...
In our case study, Na, B, and Cl were selected as the classification basis according to the hydrogeochemical mechanisms involved in geothermal system evolution. Based on well-established previous studies, some geothermal springs with relatively clear source  mechanisms were selected as typical samples, and two patterns were constructed in advance. The integrated distance from each unknown sample to the two patterns was then calculated using the ionic ratios and linear relationships among Na, B, and Cl. Each unknown sample was finally assigned to the class with the smaller integrated distance.
